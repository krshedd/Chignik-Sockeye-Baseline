...
I just wanted the allele plots of interest in one file and thought this would be easier than editing the PDF from earlier. Here, I've removed the GPDH and MHC combined markers to allow for plotting. 
```{r AlleleFreq22}
loci20plot <- loci22_RAD[-c(6,8)]
...
# Pooling with just RAD 22 loci

Here, I am running through fishers tests using ONLY the final set of 22 loci. 
...
First step is to pool temporal collections. 
```{r TemporalPool1, warning=FALSE}
ChignikTemporalPool <-
  list(
...
Due to overll pvalus, locus-specific values, and allele frequency plots I only want to combine "SCHIG98" and "SCHIG08". For all others, the markers with the highest impact are the important ones (i.e., RAD27165, U1004-183).    Further testing on pairs and geographically similar populations will follow. 
...
Now, the first non-temporal pooling test. I'll start with pairwise comparisons among early, mid, and late groups then move onto geographic locality. 
...
Overall p-values show that none of the above populations should be pooled. Also, a quick peek at the histograms below confirm this. 
...
From here, I only want to pool SALEC97 + SFAN97. The rest of the comparisons dont make sense. Especially in light of allele frequency plots and P-values. 

<b> xxx NEED TO POOL the SCHIG SAMPLES FROM ABOVE! XXXO TO DO </b>
...
None of the tests from Pool3 are great. The 'best' pairs are driven by markers that show clear differences in allele frequencies thus I chose to not pool.    
